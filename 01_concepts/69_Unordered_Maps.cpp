...
#include <iostream>
#include <unordered_map>                //  we have to import this library to use maps.
using namespace std;

int main(){

    unordered_map<string, int> m;           //  here 'string' and 'int' are 'key', 'value' pair.
    ...
    m.insert(p2);                           //  pairs are inserted in random order.
    m.insert(p3);

    for(pair<string,int> p: m){         //  elements are printed in random order.       //  instead of writing (pair<string,int> p: m), we can write (auto p: m), 'auto' keyword is used to automatically analyse the required datatype.
...
//  see leetcode 242.(valid anagram) for a better understanding of the syntax of maps.
